...
function configurarModalCambioObligatorio() {
    ...
    if (passwordConfirmar) {
        passwordConfirmar.addEventListener('input', function() {
            validarCoincidenciaObligatoria();
            // También necesitamos actualizar el estado del botón cuando cambie la confirmación
            const cumplidos = calcularCumplidosDirectamente();
...
function validarPasswordObligatoria() {
	...
	if (!passwordNueva) {
        console.error('Elemento passwordNuevaObligatoria no encontrado');
        return false;
    }
	const password = passwordNueva.value;

	// Requisitos básicos de UX
	const requisitos = {
	...
	// Calcular fuerza de contraseña (SIN LLAMAR DE VUELTA A VALIDACIONES)
...
function actualizarBarraFuerzaObligatoria(cumplidos) {
	...
	// Habilitar/deshabilitar botón (SIN LLAMAR A LAS FUNCIONES DE VALIDACIÓN)
	if (btnCambiar) {
		// Verificar manualmente los requisitos SIN LLAMAR A LAS FUNCIONES
		const passwordNuevaInput = document.getElementById('passwordNuevaObligatoria');
		const passwordConfirmarInput = document.getElementById('passwordConfirmarObligatoria');

		if (passwordNuevaInput && passwordConfirmarInput) {
			const password = passwordNuevaInput.value;
			const confirmar = passwordConfirmarInput.value;

			// Verificar requisitos básicos directamente
			const tieneRequisitos = password.length >= 8 && 
								  /[A-Z]/.test(password) && 
								  /[a-z]/.test(password) && 
								  /[0-9]/.test(password) && 
								  /[@#$%&*]/.test(password);
			...
			btnCambiar.disabled = !(tieneRequisitos && coinciden);
		}
	}
}

function calcularCumplidosDirectamente() {
    const passwordInput = document.getElementById('passwordNuevaObligatoria');
    if (!passwordInput) return 0;
    
    const password = passwordInput.value;	
    
    let cumplidos = 0;
    if (password.length >= 8) cumplidos++;
    if (/[A-Z]/.test(password)) cumplidos++;
    if (/[a-z]/.test(password)) cumplidos++;
    if (/[0-9]/.test(password)) cumplidos++;
    if (/[@#$%&*]/.test(password)) cumplidos++;
